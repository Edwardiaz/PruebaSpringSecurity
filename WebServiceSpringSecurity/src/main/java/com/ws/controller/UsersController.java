...
//	private IGenericService genS;
	...
	//@Autowired
	//public UsersController(IByIdService byIdS, IGenericService genS, IAllListService listS) {
		//this.byIdS = byIdS;
		//this.genS = genS;
		//this.listS = listS;
	//}
	...
	@ResponseBody
	public ResponseEntity<?> checker(@PathVariable("id") Long id/*@RequestHeader(name = "Authorization") String header*/) {

		Settings objetoDB = byIdS.getSettingsById(id);
//		head.add("Authorization", header);

		if (/*header.equalsIgnoreCase(objetoDB.getToken())*/ objetoDB != null) {
			return new ResponseEntity<>(listS.allProducts(),  HttpStatus.OK);
		} else {
